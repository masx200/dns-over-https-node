...
(async () => {
  ...
  try {
    // 测试1: 使用传入的强制IP参数
    console.log("\n🔹 测试1: 使用传入的强制IP参数");
    console.log("将 deno-dns-over-https-server.g18uibxgnb.de5.net 强制解析到 104.21.9.230");
    ...
    const result1 = await query({
      ...
      method: "GET",
      type: "A",
      forcedIP: "104.21.9.230", // 使用已知可用的IP
    });
    ...
    // 测试2: 使用原有的强制解析映射表
    console.log("\n🔹 测试2: 使用原有的强制解析映射表");
    console.log("使用 deno-dns-over-https-server.g18uibxgnb.de5.net (映射到 104.21.9.230)");
    ...
    console.log("\n🔹 测试3: 传入IP优先于映射表");
    console.log("将 deno-dns-over-https-server.g18uibxgnb.de5.net 强制解析到 8.8.8.8");
